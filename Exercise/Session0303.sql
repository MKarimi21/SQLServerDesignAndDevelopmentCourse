...
SELECT
    P.BusinessEntityID,
    P.FirstName + ' ' + P.MiddleName + ' ' + P.LastName [Full Name]
FROM
    [Person].[Person] AS P;
...
SELECT
    P.BusinessEntityID,
    P.FirstName + ' ' + ISNULL(P.MiddleName, '') + ' ' + P.LastName [Full Name]
FROM
    [Person].[Person] AS P;
...
SELECT
    TOP(10) YEAR(GETDATE()) - YEAR(E.BirthDate) AGE,
    *
FROM
    HumanResources.Employee E
ORDER BY
...
SELECT
    TOP(10) WITH TIES YEAR(GETDATE()) - YEAR(E.BirthDate) AGE,
    *
FROM
    HumanResources.Employee E
ORDER BY
    YEAR(E.BirthDate) DESC;
...
FROM
    Sales.SalesOrderHeader H
ORDER BY
    H.SalesOrderID OFFSET 5 ROWS FETCH NEXT 5 ROWS ONLY;
