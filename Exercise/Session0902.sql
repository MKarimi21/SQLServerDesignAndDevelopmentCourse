...
-- RECURSIVE CTE
WITH RCTE(Id, Title, ParentId, Lvl) AS (
	-- ANCHOR
	SELECT
		T1.Id,
		T1.Title,
	...
	SELECT
		...
		RCTE.Lvl + 1
	FROM
		@OrganizationChart T2
		JOIN RCTE ON RCTE.Id = T2.Parent
)
SELECT
	RCTE.*,
	T3.Title
FROM
	RCTE
	LEFT JOIN @OrganizationChart T3 ON RCTE.ParentId = T3.Id